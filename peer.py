...
class Peer():
        ...
        if len(self.peers) > 0:
            ...
            for ID in self.peers:
                ...
                if fromPeer != "not found":
                    file_exists = exists(
                        FILE_PATH + self.peerName+"/"+fromPeer)
                    if file_exists:
    ...
    def client(self):
        ...
        try:
            while True:
                print("\nAvailable Commands ")
                print("Register a File: addfile")
                print("Search for a File: search ")
                print("Obtain a File: getfile ")
                print("EXIT: exit  \n")

                clientEnter = input("Enter Command:  ")

                # register a file
                if str(clientEnter) == 'addfile':
                    if len(self.peers) == 0:
                        self.connectToPeers()

                    fileName = input("Enter filename with extension: ")
                    file_exists = exists(
                        FILE_PATH + self.peerName+"/"+fileName)
                    if file_exists:
                ...
                # obtain a file
                    ...
                    if obtainFileName in self.hashTable:
                        ...
                        if file_exists_here == True:
                            ...
                            file = open(FILE_PATH + self.peerName +
                                        "/"+obtainFileName, "wb")
                            print("Receiving....")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 3:
        ...
        print("\nUsage: python peer.py <peerName> <port>")
        os._exit(1)
